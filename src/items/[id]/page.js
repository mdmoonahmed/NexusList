...
export default async function ItemDetails({ params }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-20">
      <Link href="/items" className="text-blue-600 hover:underline mb-8 inline-block">
        ← Back to Catalog
      </Link>
      
      <div className="grid md:grid-cols-2 gap-12">
        <Image src={item.image} alt={item.name} className="w-full rounded-2xl shadow-lg" />
        <div>
          <span className="bg-blue-100 text-blue-600 px-3 py-1 rounded-full text-sm font-bold">
            {item.category}
          </span>
          <h1 className="text-4xl font-bold mt-4">{item.name}</h1>
          <p className="text-2xl font-semibold text-gray-700 mt-2">${item.price}</p>
